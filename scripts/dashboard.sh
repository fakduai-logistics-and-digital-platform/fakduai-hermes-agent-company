...
hermes_repo = Path(os.environ.get('HERMES_REPO', '/Users/watchakorn.b/FILE_CODE/hermes-fakduai-pinto')).resolve()
...
activity_serve_path = Path('data/activity.json')
...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/hermes/company-workflow':
            self.run_hermes_company_workflow()
            return
        if path == '/api/hermes/activity':
            self.receive_hermes_activity()
            return
    ...
    def run_hermes_company_workflow(self):
        try:
            size = int(self.headers.get('Content-Length', '0'))
        except ValueError:
            self.send_json_response({'ok': False, 'error': 'Invalid content length'}, 400)
        ...
        try:
            ...
            script = hermes_repo / 'scripts' / 'hermes-company-run.py'
            if not script.exists():
                raise ValueError(f'Hermes company runner not found: {script}')
            cmd = [sys.executable, str(script), task]
            if isinstance(agents, list) and agents:
                cmd.extend(['--agents', *[str(a) for a in agents if str(a).strip()]])
            started = time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())
            proc = subprocess.run(cmd, cwd=str(hermes_repo), text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=900)
            output = proc.stdout[-12000:]
            event = {
                'id': f'hermes-{int(time.time())}',
                'type': 'hermes_company_workflow',
                'from': 'dashboard',
                'to': 'hermes',
                'task': task,
            ...
            self.send_json_response({'ok': proc.returncode == 0, 'returncode': proc.returncode, 'output': output, 'event': event}, 200 if proc.returncode == 0 else 500)
        except Exception as exc:
            self.send_json_response({'ok': False, 'error': str(exc)}, 500)

    def append_activity(self, event):
        try:
            data = {'activities': [], 'workflows': []}
            if activity_serve_path.exists():
                data = json.loads(activity_serve_path.read_text(encoding='utf-8'))
            data.setdefault('activities', [])
            data['activities'].insert(0, event)
            data['activities'] = data['activities'][:100]
            activity_serve_path.parent.mkdir(parents=True, exist_ok=True)
            activity_serve_path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
        except Exception:
            pass

    def receive_hermes_activity(self):
        try:
            size = int(self.headers.get('Content-Length', '0'))
        except ValueError:
            self.send_json_response({'ok': False, 'error': 'Invalid content length'}, 400)
        ...
        try:
            ...
            activity_root = Path(os.environ.get('ACTIVITY_ROOT', str(Path.home() / '.openclaw' / 'shared' / 'company-activity')))
            shared_root = Path(os.environ.get('SHARED_ROOT', str(Path.home() / '.openclaw' / 'shared' / 'agents')))
            ...
            if event_type == 'workflow_completed':
                # Mark the whole visible team done so old recent tasks do not keep cards in Working.
                for status_dir in shared_root.iterdir() if shared_root.exists() else []:
            ...
            self.append_activity(event)
            self.send_json_response({'ok': True, 'event': event})
        except Exception as exc:
            self.send_json_response({'ok': False, 'error': str(exc)}, 500)
